...
class ZappaCLI(object):
    ...
    def deploy(self):
        ...
        success = self.zappa.upload_to_s3(
            ...
            source_path=self.handler_path
        )

        project_success = self.zappa.upload_to_s3(
            bucket_name=self.s3_bucket_name,
            source_path=self.zip_path
        )
        if not success or not project_success: # pragma: no cover
    ...
    def update(self):
        ...
        self.lambda_arn = self.zappa.update_lambda_function(
            self.s3_bucket_name, self.handler_path, self.lambda_name)
